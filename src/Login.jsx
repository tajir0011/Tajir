...
function Login({ onSuccess, onSignUp, lang = "en" }) {
    ...
    <div style={{ minHeight: "100vh", display: "flex", fontFamily: "'Segoe UI', Arial, sans-serif", direction: isArabic ? "rtl" : "ltr" }}>
      ...
      <div style={{ flex: 1, display: "flex", alignItems: "center", justifyContent: "center", padding: "60px", background: "white" }}>
        <div style={{ width: "100%", maxWidth: "400px" }}>
          <h2 style={{ fontSize: "28px", fontWeight: "800", color: DARK, margin: "0 0 8px" }}>{txt.loginTitle}</h2>
          <p style={{ color: "#888", margin: "0 0 32px", fontSize: "15px" }}>{txt.loginSubtitle}</p>
          ...
          <p style={{ textAlign: "center", color: "#888", fontSize: "13px", marginTop: "20px" }}>
            {txt.noAccount}{" "}
            <span onClick={onSignUp} style={{ color: CORAL, cursor: "pointer", fontWeight: "600" }}>{txt.signUpLink}</span>
          </p>
        </div>
      </div>
    </div>
